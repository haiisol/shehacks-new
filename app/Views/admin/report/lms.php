...
<div class="row mt-3">
        ...
        <div class="card">
            <div class="card-body">
                <div class="d-flex align-items-start flex-wrap gap-2">
                    ...
                    <div>
                        <p class="mb-1">Total User Kunjungi LMS</p>
                        <h4 id="load_total_user"></h4>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="col-md-4">
        <div class="card">
            <div class="card-body">
                <div class="d-flex align-items-start flex-wrap gap-2">
                    <a href="javascript:void(0)" class="widget-icon-2 rounded-10 bg-light-primary text-primary me-3">
                        <ion-icon name="book-outline"></ion-icon>
                    </a>
                    <div>
                        <p class="mb-1">Total Modul Dikunjungi</p>
                        <h4 id="load_total_modul"></h4>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card">
            <div class="card-body">
                <div class="d-flex align-items-start">
                    ...
                    <div class="mb-3">
                        <h6>Pengunjung modul</h6>
                        <p>Berdasarkan user pengunjung</p>
                    </div>
                </div>
                <div class="text-center" id="no-data-pengunjung">
                    <img src="<?php echo base_url();?>assets/backoffice/images/no-data.png" class="image-no-data">
                </div>
                <div class="chart-container6" id="container-chart-pengunjung">
                    <canvas id="chart_url_pengunjung"></canvas>
                </div>
            </div>
        </div>
    </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        var table_data = $('#table-data').DataTable({
            ajax: {
                url : '<?php echo base_url();?>admin/report/lms/datatables',
                complete: function(data, type) {
                    json = data.responseJSON;
                    $('#load_total_user').html(json.total_user+' <span class="fw-400 fs-6">Users</span>');
                    $('#load_total_modul').html(json.total_modul+' <span class="fw-400 fs-6">Modul</span>');
                    $('#load_total_video').html(json.total_video+' <span class="fw-400 fs-6">Video</span>');
                },
            },
            columnDefs : [
                {
        ...
        $('#form-filter').submit(function(e) {
            ...
            url_reload = '<?php echo base_url();?>admin/report/lms/datatables/?' + filter;
        ...
        var chart_pengunjung = new Chart(document.getElementById('chart_url_pengunjung'), {
            type: 'pie',
            data: {
                datasets: [{
                    label: ' ',
        ...
        load_data_url_pengunjung('');
            
        function load_data_url_pengunjung(filter) {
            $.ajax({
                url  : '<?php echo base_url();?>admin/report/lms/load_data_pengunjung?'+filter,
                data : { param:'pengunjung' },
                success: function(response) {
                    if (response.status == 1) {
                        document.getElementById("container-chart-pengunjung").style.display="block";
                        $('#no-data-pengunjung').addClass('d-none');
                        
                        if (response.labels.length > 0) {
                            chart_pengunjung.data.labels            = response.labels;
                            chart_pengunjung.data.datasets[0].data  = response.datasets;
                            chart_pengunjung.update();
                        } else {
                            document.getElementById("container-chart-pengunjung").style.display="none";
                            $('#no-data-pengunjung').removeClass('d-none');
                        }
            ...
            function load_total_date(value) {
                $.ajax({
                    url  : '<?php echo base_url();?>admin/report/lms/fetch_data_total_date',
                    data : { value:value },
                    success: function(response) {
                        if (response.status == 1) {
