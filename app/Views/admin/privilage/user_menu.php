            ...
            <div class="modal-body">
                <form method="post" id="form-data" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="nama_menu" id="nama_menu" placeholder="" class="form-control">
                    </div>

                    <div class="form-group">
                        <label class="control-label">Kode Menu<span class="text-danger">*</span></label>
                        <input type="text" name="kode_menu" id="kode_menu" placeholder="" class="form-control">
                    </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        var table_data = $('#table-data').DataTable({
            ajax: {
                url : '<?php echo base_url();?>admin/privilage/user_menu/datatables',
            },
            columnDefs : [
                {
        ...
        $.validator.addMethod('cek_value', function(value, element) {
            $.ajax({
                method   : 'post',
                url      : '<?php echo base_url();?>admin/privilage/user_menu/cek_value',
                data     : { value:value },
                dataType : 'json',
        ...
        $.validator.addMethod('cek_value_code', function(value, element) {
            $.ajax({
                method   : 'post',
                url      : '<?php echo base_url();?>admin/privilage/user_menu/cek_value_code',
                data     : { value:value },
                dataType : 'json',
                success: function(response) {
                    if (response.status == 1) {
        ...
        var validate_form = $('#form-data').validate({
                ...
                nama_menu: {
                    ...
                    cek_value: {
                        depends: function(element) {
                            if ($param_input.val() == "edit") { return false; } else { return true; }
                        }
                    }
                },
                kode_menu: {
            ...
            messages: {
                nama_menu: {
                    ...
                    cek_value: 'Nama menu sudah digunakan'
                },
                kode_menu: {
                    required: 'Kode menu harus diisi.',
                    cek_value_code: 'Kode menu sudah digunakan'
                },
            },
            errorElement: 'em',
        ...
        // --------------------------- add & edit data --------------------------- 
            ...
            $(document).on('click', '.edit-data', function() {
                ...
                $.ajax({
                    type     : 'POST',
                    url      : '<?php echo base_url(); ?>admin/privilage/user_menu/get_data',
                    data     : { id:id },
                    dataType : 'json',
                    success: function(response) {
                        $('#id').val(response.id_menu);
                        $('#nama_menu').val(response.nama_menu);
                        $('#kode_menu').val(response.kode_menu);
                    }
                });
                return false;
            });
            ...
            $('#form-data').submit(function(e) {
                ...
                if (validate_form.valid()) {
                    ...
                    if(save_method == 'add') {
                        url = '<?php echo base_url(); ?>admin/privilage/user_menu/add_data';
                    } else {
                        url = '<?php echo base_url(); ?>admin/privilage/user_menu/edit_data';
                    }
        ...
        // --------------------------- delete data ---------------------------
            ...
            $(document).on('click', '#button-delete', function() {
                ...
                $.ajax({
                    type : 'POST',
                    url  : '<?php echo base_url();?>admin/privilage/user_menu/delete_data',
