...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        var table_data = $('#table-data').DataTable({
            ajax: {
                url : '<?php echo base_url();?>admin/master/sector/datatables',
            },
            columnDefs : [
                {
        ...
        $.validator.addMethod('cek_value', function(value, element) {
            $.ajax({
                method   : 'post',
                url      : '<?php echo base_url();?>admin/master/sector/cek_value',
                data     : { value:value },
                dataType : 'json',
        ...
        var validate_form = $('#form-data').validate({
            ...
            messages: {
                nama: {
                    required: 'Sector harus diisi.',
                    cek_value: 'Sector sudah digunakan'
                },
            },
            errorElement: 'em',
        ...
        // --------------------------- add & edit data --------------------------- 
            ...
            $(document).on('click', '.edit-data', function() {
                ...
                $.ajax({
                    type     : 'POST',
                    url      : '<?php echo base_url(); ?>admin/master/sector/get_data',
                    data     : { id:id },
                    dataType : 'json',
                    success: function(response) {
            ...
            $('#form-data').submit(function(e) {
                ...
                if (validate_form.valid()) {
                    ...
                    if(save_method == 'add') {
                        url = '<?php echo base_url(); ?>admin/master/sector/add_data';
                    } else {
                        url = '<?php echo base_url(); ?>admin/master/sector/edit_data';
                    }
        ...
        // --------------------------- delete data ---------------------------
            ...
            $(document).on('click', '#button-delete', function() {
                ...
                $.ajax({
                    type : 'POST',
                    url  : '<?php echo base_url();?>admin/master/sector/delete_data',
