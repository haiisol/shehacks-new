        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="post" id="form-data" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        <label class="control-label">Kode Youtube<span class="text-danger">*</span></label>
                        <input type="text" name="kode_youtube" id="kode_youtube" class="form-control" placeholder="Silahkan masukkan Kode youtube saja">
                    </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        var table_data = $('#table-data').DataTable({
            ajax: {
                url      : '<?php echo base_url();?>admin/webinar/webinar/datatables',
            },
            columnDefs : [
                {
        ...
        $.validator.addMethod('cek_value', function(value, element) {
            $.ajax({
                method   : 'post',
                url      : '<?php echo base_url();?>admin/webinar/webinar/cek_value',
                data     : { value:value },
                dataType : 'json',
        ...
        var validate_form = $('#form-data').validate({
            ...
            messages: {
                judul: {
                    required: 'Judul Webinar harus diisi.',
                    cek_value: 'Judul Webinar sudah digunakan'
                },
                kode_youtube: {
                    required: 'Kode Youtube Webinar harus diisi.',
                }
            },
            errorElement: 'em',
        ...
        // --------------------------- add & edit data --------------------------- 
            ...
            $(document).on('click', '.edit-data', function() {
                ...
                $.ajax({
                    type     : 'POST',
                    url      : '<?php echo base_url(); ?>admin/webinar/webinar/get_data',
                    data     : { id:id },
                    dataType : 'json',
                    success: function(response) {
                        $('#id').val(response.id_webinar);
                        $('#judul').val(response.judul);
                        $('#kode_youtube').val(response.kode_youtube);
                    }
                });
                return false;
            });
            ...
            $('#form-data').submit(function(e) {
                ...
                if (validate_form.valid()) {
                    ...
                    if(save_method == 'add') {
                        url = '<?php echo base_url(); ?>admin/webinar/webinar/add_data';
                    } else {
                        url = '<?php echo base_url(); ?>admin/webinar/webinar/edit_data';
                    }
        ...
        // --------------------------- delete data ---------------------------
            ...
            $(document).on('click', '#button-delete', function() {
                ...
                $.ajax({
                    type : 'POST',
                    url  : '<?php echo base_url();?>admin/webinar/webinar/delete_data',
