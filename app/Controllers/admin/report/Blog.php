...
class Blog extends CI_Controller {
    ...
    {   
        ...
        $data['title']       = 'Data Pengunjung Blog';
        $data['description'] = '';
        $data['keywords']    = '';
        $data['page']           = 'admin/report/blog';
    ...
    function _sql($group_by) 
    {
        ...
        $this->db->select('
            ...
            SUM(a.hits) as total
        ');
        $this->db->from('tb_analytic_blog a USE INDEX (blog_report)');  
        $this->db->join('tb_blog b', 'a.id_blog = b.id_blog', 'left');   
    ...
    {   
        ...
        $this->main_model->datatable_custom($valid_columns, ' a.id_blog ', ' total DESC ');
        ...
        foreach($query->result_array() as $key) {
            ...
            $data[] = array(
                $no,
                '<a href="'.$url_detail.'" target="_blank">'.character_limiter($key['judul'], 55).'</a>',
        ...
        $response['recordsTotal']     = $this->_sql(' a.id_blog ')->num_rows();
        $response['recordsFiltered']  = $this->_sql(' a.id_blog ')->num_rows();
    ...
    function _sql_total($param) 
    {
        ...
        if ($param == 'pengunjung') {
            ...
            $select = ' COUNT(DISTINCT a.referral) as total ';
        }
    ...
    function load_data_url_referal()
    {   
        $this->main_model->check_access('report');
        $cact = $this->main_model->check_access_action('report');
        ...
        if ($cact['access_view'] == 'd-none') {
            ...
            $get_data = $this->db->query("
                SELECT IFNULL(a.referral, '-') as nama, COUNT(a.referral) as total
                FROM tb_analytic_blog a USE INDEX (blog_report) 
                ".$where."
                GROUP BY a.referral 
