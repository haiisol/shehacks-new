...
class Intro extends CI_Controller {
    ...
    {
        $data = $this->main_model->check_access('content_program');
        $cact = $this->main_model->check_access_action('content_program');
        ...
        $data['description'] = '';
        $data['keywords']    = '';
        $data['page']           = 'admin/content/program/intro';
        $this->load->view('admin/index', $data);
    }

    function get_data()
    {   
        $this->main_model->check_access('content_program');
        $cact = $this->main_model->check_access_action('content_program');
        ...
        if ($cact['access_view'] == 'd-none') {
            ...
            $query = $this->db->query("SELECT * FROM tb_content WHERE section = 'program_intro' ")->row_array();
    ...
    {   
        ...
        if ($cact['access_edit'] == 'd-none') {
            ...
            $this->form_validation->set_rules('subheading', 'subheading', 'trim|required');
            ...
            $data['subheading']         = sanitize_input($subheading);
            ...
            $data['content']            = sanitize_input_textEditor($this->input->post('content', TRUE));
