...
class Cta_btn extends CI_Controller {
    ...
    {   
        ...
        $data['title']       = 'Data Pengunjung CTA Button';
        $data['description'] = '';
        $data['keywords']    = '';
        $data['page']           = 'admin/report/cta';
    ...
    function _sql($group_by) 
    {
        ...
        $this->db->select('
            ...
            SUM(a.hits) as total
        ');
        $this->db->from('tb_analytic_cta a USE INDEX (cta_report)');  

        if ($fil_date) {
            $this->db->where('a.date_create BETWEEN "'.$tgl_start_query .'" AND "'.$tgl_end_query.'" ');
        }
    ...
    {   
        ...
        $this->main_model->datatable_custom($valid_columns, ' a.page_view, a.cta', ' total DESC ');
        ...
        foreach($query->result_array() as $key) {
            ...
            $data[] = array(
                $no,
                character_limiter($key['page_view'], 55),
                character_limiter($key['cta'], 55),
                number_format($key['total'], 0,",","."),
            );
        }

        $total_pengunjung = $this->_sql_total('pengunjung');
        $total_referral   = $this->_sql_total('referral');
        
        $response['draw']              = intval($this->input->get('draw'));
        $response['recordsTotal']      = $this->_sql(' a.page_view, a.cta')->num_rows();
        $response['recordsFiltered']   = $this->_sql(' a.page_view, a.cta')->num_rows();
        $response['total_pengunjung']  = $total_pengunjung;
        $response['total_referral']    = $total_referral;
        $response['data']              = $data;
    ...
    function _sql_total($param) 
    {
        ...
        if ($param == 'pengunjung') {
            $select = ' SUM(a.hits) as total ';
        } else {
            $select = ' COUNT(DISTINCT a.ip_address) as total ';
        }
    ...
    {   
        ...
        if ($cact['access_view'] == 'd-none') {
            ...
            $fil_date       = trim($this->input->get('fil_date'));
            $param          = trim($this->input->get('param', TRUE));
            ...
            if ($fil_date) {
                ...
                $where = ' WHERE a.date_create BETWEEN "'.$tgl_start_query .'" AND "'.$tgl_end_query.'" ';
            }

            if ($param == 'pv') {
                $select = " IFNULL(a.page_view, '-') as nama, ";
                $group  = " a.page_view  ";
            } else {
                $select = " IFNULL(a.cta, '-') as nama, ";
                $group  = " a.cta  ";
            }
            

            $get_data = $this->db->query("
                SELECT ".$select." SUM(a.hits) as total
                FROM tb_analytic_cta a USE INDEX (cta_report) 
                ".$where."
                GROUP BY ".$group."
    ...
    {   
        ...
        if ($cact['access_view'] == 'd-none') {
            ...
            if ($value == 'last7') {
                ...
                $label_title = 'Total Hits 7 Hari Terakhir';
                ...
                foreach ($array_date as $key) {
                    ...
                    $get = $this->db->query("
                        SELECT SUM(a.hits) as total 
                        FROM tb_analytic_cta a USE INDEX (cta_report)
                        WHERE DATE(a.date_create) = '".$key."' ")->row_array();
                ...
                $label_title = 'Total Hits Bulan Ini';
                ...
                foreach ($array_date as $key => $val) {
                    ...
                    $get = $this->db->query("
                        SELECT SUM(a.hits) as total 
                        FROM tb_analytic_cta a USE INDEX (cta_report)
                        WHERE DAY(a.date_create)='".$val."' 
                        AND MONTH(a.date_create)='".$month."' 
                        AND YEAR(a.date_create)='".$year."' ")->row_array();
                ...
                $label_title = 'Total Hits Tahun Ini';
                ...
                $label_title = 'Total Hits Hari Ini';
                ...
                foreach ($array_date as $key => $val) {
                    ...
                    $get = $this->db->query("
                        SELECT SUM(a.hits) as total 
                        FROM tb_analytic_cta a USE INDEX (cta_report)
                        WHERE (TIME(a.date_create) > '".$val.":00' 
                        AND TIME(a.date_create) < '".$val.":59')
                        AND DAY(a.date_create) = '".$days."'
                        AND MONTH(a.date_create) ='".$month."' 
                        AND YEAR(a.date_create) ='".$year."' ")->row_array();
